...
<head>
    ...
    <style>
        *{margin:0;padding:0;box-sizing:border-box;}
        ...
        .sb-brand-beauty{
            font-family:'Playfair Display',serif;
            font-size:1.6rem;font-weight:900;font-style:italic;line-height:1;
            background:linear-gradient(135deg,var(--purple) 0%,var(--pink) 100%);
            -webkit-background-clip:text;-webkit-text-fill-color:transparent;
            background-clip:text;position:relative;z-index:1;
        }
        .sb-brand-sub{
            font-size:.62rem;color:var(--sb-muted);
            letter-spacing:.14em;text-transform:uppercase;
            font-weight:600;margin-top:.2rem;position:relative;z-index:1;
        }
        .sb-brand-pill{
        ...
        .sb-nav::-webkit-scrollbar-thumb{background:rgba(124,58,237,.12);}
        ...
        .sb-link:hover .sb-ico{background:rgba(124,58,237,.12);color:var(--purple);}
        .sb-link.active{
        ...
        .sb-link.soon{opacity:.55;cursor:default;}
        ...
        .sb-soon-pill{
            ...
            border-radius:10px;padding:.1rem .38rem;
            letter-spacing:.05em;
        }
        ...
        .sb-foot{
            ...
            border-top:1.5px solid var(--sb-border);flex-shrink:0;
            background:#faf7ff;
        }
        .sb-user{
            display:flex;align-items:center;gap:.7rem;
            padding:.58rem .72rem;border-radius:11px;
            background:#fff;border:1.5px solid var(--sb-border);
        ...
        .sb-uname{display:block;font-size:.8rem;color:var(--text);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;font-weight:600;}
        ...
        .topbar{
            ...
            box-shadow:0 1px 12px rgba(124,58,237,.06);
        }
        .topbar-left{flex:1;}
        .topbar-title{font-family:'Playfair Display',serif;font-size:1.1rem;font-weight:700;color:var(--text);line-height:1;}
        ...
        .tb-icon:hover{background:var(--purple-soft);border-color:rgba(124,58,237,.3);color:var(--purple);}
        .tb-dot{
            position:absolute;top:7px;right:7px;
            width:7px;height:7px;background:var(--tango);
            border-radius:50%;border:1.5px solid #fff;
        }
        .tb-divider{width:1px;height:26px;background:var(--border);}

        /* ── User dropdown — matches screenshot style ── */
        .tb-user{
        ...
        .tb-user:hover{background:var(--purple-soft);border-color:rgba(124,58,237,.25);}
        .tb-av{
        ...
        .tb-chev{font-size:.65rem;color:var(--muted);transition:transform .2s;}
        .tb-user.open .tb-chev{transform:rotate(180deg);}

        /* Dropdown — clean white card like screenshot */
        .tb-drop{
            display:none;position:absolute;
            top:calc(100% + .7rem);right:0;
            background:#fff;
            border:1.5px solid var(--border);
            border-radius:16px;
            box-shadow:0 8px 32px rgba(124,58,237,.14),0 2px 8px rgba(0,0,0,.06);
        ...
        /* Header row — name + email like screenshot */
        .tb-drop-head{
            padding:1rem 1.1rem .9rem;
            border-bottom:1px solid #f3eeff;
        }
        .tb-drop-head strong{display:block;font-size:.9rem;font-weight:700;color:var(--text);}
        .tb-drop-head span{font-size:.76rem;color:var(--muted);}

        .tb-drop a,.tb-drop button{
            display:flex;align-items:center;gap:.7rem;
            padding:.72rem 1.1rem;font-size:.84rem;color:var(--text);
            background:none;border:none;width:100%;cursor:pointer;
            font-family:inherit;transition:background .13s;
        }
        .tb-drop a i{color:var(--purple);width:16px;text-align:center;font-size:.88rem;}
        .tb-drop a:hover{background:#f8f5ff;color:var(--purple);}
        .tb-drop button i{color:var(--tango);width:16px;text-align:center;font-size:.88rem;}
        .tb-drop button{color:var(--tango);font-weight:500;}
        .tb-drop button:hover{background:var(--tango-soft);}
        .tb-drop hr{border:none;border-top:1px solid #f3eeff;margin:.2rem 0;}
        ...
        .card-header{
            ...
            background:linear-gradient(120deg,#fff 45%,var(--purple-soft) 100%);flex-wrap:wrap;
        }
...
<aside class="sidebar">
    ...
    <nav class="sb-nav">
        ...
        {{-- M-Pesa Logs — ready when route exists --}}
        <a href="{{ route_exists('admin.logs.mpesa') ? route('admin.logs.mpesa') : '#' }}"
           class="sb-link {{ request()->routeIs('admin.logs.mpesa') ? 'active':'' }}">
            ...
            @unless(route_exists('admin.logs.mpesa'))
        ...
        <a href="{{ route_exists('admin.logs.customers') ? route('admin.logs.customers') : '#' }}"
           class="sb-link {{ request()->routeIs('admin.logs.customers') ? 'active':'' }}">
            ...
            @unless(route_exists('admin.logs.customers'))
                <span class="sb-soon-pill">Soon</span>
            @endunless
        </a>
        ...
        <a href="{{ route_exists('admin.logs.managers') ? route('admin.logs.managers') : '#' }}"
           class="sb-link {{ request()->routeIs('admin.logs.managers') ? 'active':'' }}">
            ...
            @unless(route_exists('admin.logs.managers'))
                <span class="sb-soon-pill">Soon</span>
            @endunless
        </a>
        ...
        <a href="{{ route_exists('admin.logs.pos') ? route('admin.logs.pos') : '#' }}"
           class="sb-link {{ request()->routeIs('admin.logs.pos') ? 'active':'' }}">
            ...
            @unless(route_exists('admin.logs.pos'))
                <span class="sb-soon-pill">Soon</span>
            @endunless
        </a>
...
<div class="main">
    ...
    <div class="topbar">
        ...
        <div class="topbar-right">
            ...
            <a href="{{ route('admin.pos.index') }}" class="tb-icon" title="POS Terminal">
            ...
            <div class="tb-user" id="tbUser" onclick="toggleDrop()">
                ...
                <div class="tb-drop" id="tbDrop">
                    ...
                    <div class="tb-drop-head">
                        ...
                        <span>{{ auth()->user()->email ?? 'admin@americanbeauty.com' }}</span>
                    </div>
                    <a href="#"><i class="fas fa-user-pen"></i> Edit Profile</a>
                    <a href="#"><i class="fas fa-clock-rotate-left"></i> Order History</a>
                    <a href="#"><i class="fas fa-circle-info"></i> Account Info</a>
